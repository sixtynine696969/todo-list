...
const displayController = (function () {
  ...
  const getButtons = () =>
    document.querySelectorAll(".sidebar > button:not(button.add-list)");
  ...
  const removeAllCardsChildren = () =>
    getCardsEelemtChildren().forEach((i) => i.remove());
      ...
      expandButton.addEventListener("click", () => {
        ...
        if (!card.querySelector(".invisible")) {
          innerDivs.forEach((div) => {
            if (
              !(
                div.classList.contains("dueDate") ||
  ...
  const removeRemoveButton = () =>
    document.querySelectorAll(".remove-button").forEach((i) => i.remove());
  ...
  const handleAddItemButtons = () => {
    const submitButton = document.querySelector(
      ".add-item-form .add-item-submit",
    );
    const closeButton = document.querySelector(
      ".add-item-form .add-item-close",
    );
  ...
  const handleAddListButtons = () => {
    const submitButton = document.querySelector(
      ".add-list-window .add-list-submit",
    );
    const closeButton = document.querySelector(
      ".add-list-window .add-list-close",
    );
...
const todoList = (function () {
  ...
  list = {
    inbox: [
      ...
      new todoItem(
        "Brush teeth",
        "I got a new tooth paste to try out",
        "2023-11-28",
        "low",
        "best to brush them while pooping to not waste time",
      ),
      new todoItem(
        "Buy gucci flip flops",
        "gotta flex on dem ho*s",
        "2024-11-31",
        "urgent",
        "buy a an off white tee for double flexerino",
      ),
    ],
    today: [new todoItem("Learn JavaScript", "", "2023-11-01", "mid", "")],
    week: [new todoItem("Clean my room", "", "2023-08-01", "mid", "")],
  };
